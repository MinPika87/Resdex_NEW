...
class MatrixFeatures():
    def __init__(self):
        self.base_dir = "/data/analytics/rohit.agarwal/ResDex_Agent/Matrices/feature_matrices/"

        print("Loading Matrix Features")
        start = time()

        self.skillToSkillFeature = Feature2dMatrix('query_skill_id', 'cand_skill_id', 'score',
                                                   file_path=self.base_dir + "skillToSkillFeature", thres=200)
        self.titleToSkillFeature = Feature2dMatrix('query_title_id', 'cand_skill_id', 'score',
                                                   file_path=self.base_dir + "titleToSkillFeature", thres=50)
        self.skillToTitleFeature = Feature2dMatrix('query_skill_id', 'cand_desig_id', 'score',
                                                   file_path=self.base_dir + "skillToTitleFeature", thres=50)
        self.titleToTitleFeature = Feature2dMatrix('query_title_id', 'cand_desig_id', 'score',
                                                   file_path=self.base_dir + "titleToTitleFeature", thres=5)

        self.size = (
            self.skillToSkillFeature.get_size() +
            self.titleToSkillFeature.get_size() +
            self.skillToTitleFeature.get_size() +
